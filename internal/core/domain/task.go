...
func (p *TaskPatch) Validate() error {
	if p.Title.Set && p.Title.Value == nil {
		return fmt.Errorf("'title' can't be null if set: %w", core_errors.ErrInvalidArgument)
	}
	if p.Completed.Set && p.Completed.Value == nil {
		return fmt.Errorf("'completed' can't be null if set: %w", core_errors.ErrInvalidArgument)
	}
	return nil
}
